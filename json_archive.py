...
class JsonArchive:
    ...
    def write(self, payload):
        ...
        No-op (returns None) when archiving is disabled. Failures are logged but
        never raised — archiving must not break the collection cycle.
        """
        if not self.enabled:
            return None
        ...
        base = f"{self.program_name}_{now.strftime('%Y%m%d_%H%M%S')}_{time.time_ns() % 1000:03d}"
    ...
    def _prune(self):
        ...
        # 2) Any leftover loose .json inside (un-rolled) day folders.
        for root, _dirs, files in os.walk(self.directory):
            for name in files:
                if not (name.startswith(self.program_name) and name.endswith(".json")):
                    continue
